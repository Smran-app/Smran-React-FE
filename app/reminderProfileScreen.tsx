...
function mapUiProfileToApiPayload(
...
export default function ReminderProfileScreen() {
  ...
  const [editingProfile, setEditingProfile] = useState<ReminderProfile | null>(
    null,
  );
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (isApiProfileId(editingProfile.id)) {
        ...
        setProfiles((prev) =>
          prev.map((p) =>
            p.id === editingProfile.id ? mapApiProfileToUi(updated) : p,
          ),
        );
      } else {
